...
cognitiveServiceNameCurrent=$(az cognitiveservices account list --resource-group ai102-cognitive-rg --query "[?contains(name,'ai102-cognitive-$serviceType')].name" --output tsv)
#if service name exists, keep it
if [ -z "$cognitiveServiceNameCurrent" ]
then
    #create variable random name for cognitive service
    cognitiveServiceNamePrevious=$(az cognitiveservices account list --resource-group ai102-cognitive-rg --query "[?contains(name,'ai102-cognitive')].name" --output tsv)
